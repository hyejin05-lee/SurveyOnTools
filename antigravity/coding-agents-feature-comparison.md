...
| ├ 에이전트 팀 오케스트레이션 | ● [6] | ◐ | ● [17] | ◐ [23, 58, 64] | ○ | ● [36] | ◐ |
| └ 의도 분류/라우팅 레이어 | ○ | ○ | ○ | ○ | ○ | ● [37] | ○ |
| **5. 멀티모델 라우팅** | | | | | | | |
...
한편, SDK의 `multi-agent.ts`(1,846줄) 코드 분석 결과 [64], Cline은 비교 대상 중 가장 풍부한 **팀 오케스트레이션 인프라**를 SDK 레벨에서 제공하고 있다. `AgentTeamsRuntime` 클래스는 `lead`(리드) + `teammate`(팀원) 계층 구조를 네이티브로 구현하며, 다음 기능을 포함한다:
- **태스크 관리:** `createTask`, `claimTask`, `blockTask`, `completeTask` + `dependsOn` 기반 의존성 해석으로 작업 순서를 보장
- **메일박스 메시징:** `sendMessage`, `broadcast`로 에이전트 간 비동기 통신, 읽음/안읽음 추적 및 실행 중 teammate에게 `pendingSteerMessage` 동적 주입(실시간 조향)
- **미션 로그:** 팀 전체 활동의 시간순 구조화 로깅(progress/done/error/blocked), 간격 기반 자동 기록
- **비동기 런 큐:** `startTeammateRun`으로 우선순위 기반 큐잉, `maxConcurrentRuns` 동시성 제어, 지수 백오프 재시도, `recoverActiveRuns`를 통한 중단 후 자동 복구
- **구조화된 산출물(Outcome) 관리:** `createOutcome` → 섹션별 `attachOutcomeFragment` → `reviewOutcomeFragment`(승인/반려) → `finalizeOutcome` 파이프라인으로, 팀 공동 산출물의 라이프사이클을 관리
- **팩토리 패턴:** `createWorkerReviewerTeam`으로 작업-검토 패턴을 원샷 생성, `runParallel`·`runSequential`·`runPipeline` 3가지 실행 전략 제공

그러나 에이전트 팀 오케스트레이션은 **부분 지원(◐)**으로 분류한다. 이유는 ① oh-my-openagent의 11종 사전 정의 전문 에이전트(Sisyphus, Prometheus 등)와 달리 역할이 `"lead" | "teammate"` 2종으로 고정되어 전문 에이전트 프리셋이 부재하며, ② 사용자 요청을 자동 분류하여 최적 에이전트로 라우팅하는 IntentGate 같은 의도 분류/라우팅 레이어가 없고(`routeToTeammate`는 명시적 agentId 지정 필수), ③ 이 인프라가 SDK 레벨의 "빌드 블록"에 머물러 최종 사용자 제품(VS Code 확장)에서 바로 사용 가능한 빌트인 팀 구성으로 노출되지 않기 때문이다 [64].
...
2. **오케스트레이션 성숙도:** oh-my-openagent > Claude Code ≈ Antigravity > Cline ≈ Hermes Agent > Codex CLI > OpenClaw (Cline은 SDK 인프라 성숙도가 높으나 빌트인 전문 에이전트 팀 부재로 제품 레벨에서는 ◐)
...
[64] Cline, "Cline SDK — AgentTeamsRuntime: lead/teammate hierarchical team orchestration, task management, mailbox messaging, mission logging, async run queue & outcome/fragment lifecycle," sdk/packages/core/src/extensions/tools/team/multi-agent.ts. 코드 직접 분석(1,846줄): AgentTeam(routeTo/runParallel/runSequential/runPipeline), AgentTeamsRuntime(spawnTeammate/routeToTeammate/startTeammateRun/createTask/claimTask/blockTask/completeTask + dependsOn 의존성 해석 + 메일박스 sendMessage/broadcast + 미션 로그 + 우선순위 런 큐 maxConcurrentRuns + Outcome/Fragment 산출물 라이프사이클), createWorkerReviewerTeam 팩토리. 역할은 "lead"|"teammate" 2종 고정, IntentGate급 자동 라우팅 부재.

---

> **Caveat:** 특수목적 에이전트 3종(OpenClaw, oh-my-openagent, Hermes Agent)에 대한 일부 정보는 공식 문서 및 GitHub README, 커뮤니티 블로그 포스트에 기반하며, 공개 문서가 제한적인 경우 웹 검색 기반 추정이 포함될 수 있다. 각 인용의 정확성은 해당 출처의 접근 가능 여부에 따라 달라질 수 있다. **Claude Code(`claude-code-main/`), Cline(`cline-main/sdk/`), Hermes Agent(`hermes-agent-main/`)는 AGENTS.md §6에 따라 문서가 아닌 소스코드를 직접 분석하여 판정하였다.** 참조 번호 [47]~[64]는 소스 파일 기반 코드 검증 출처이다.
